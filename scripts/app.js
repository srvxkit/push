...
function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--create') isCreate = true;
  ...
  // Default to --list if no flags passed or --list passed
  if (isList || (!isCreate && !name)) {
    const apps = appRepo.findAll();
    console.log('========================================================================================');
    console.log('📋 Registered Applications');
    console.log('========================================================================================');
    if (apps.length === 0) {
      console.log('No applications registered yet. Run: npm run app -- --create --name "My App"');
    } else {
      apps.forEach(app => {
